...
public class RandomNumberPattern {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int test_case = 1; test_case <=T; test_case++) {
			...
			//구현하기
			//1. 행렬 순회하며 상하좌우 탐색하기 (가장자리는 제외)			
			
			for (int r = 1; r < N - 1; r++) { //행탐색 (경계를 검사할 필요가 없다)
				for (int c = 1; c < N - 1; c++) { //열탐색
					//행렬 보다 A가 크다면
//					boolean countA1 = true;
//					boolean countB1 = true; boolean으로 탐색한느 것은 안좋음 대신 if 조건 쓰기
					int big_count = 0;
					int small_count = 0;
			...
			System.out.println("#"+test_case+" "+big+" "+small);
				...
				//1.3 행렬 안의 상하좌우 좌표를 얻고 경계내에 있는지 검사 
				//1.4 상하좌우에 커버됐다고 표시
			//2. 맵 전체 탐색하면서 가장자리 개수를 센다.
