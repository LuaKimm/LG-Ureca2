...
/**
 * List : - 데이타를 저장한 순서대로 저장해 준다. 
 * - 데이타의 Index는 0부터 size()-1이다 - 중간 삽입할 수 있는
 * Index는 0부터 size()까지 이다. 
 ...
 * 중간 삽입 없이 맨끝에 추가해서 탐색만 하는거면 ArrayList
 * 
 */
...
//		list.add(6, 256);		//IndexOutOfBoundsException 발생. 
		 ...
		 * - 선언과 생성시 저장할 타입를 지정 => 지정한 타입 외에는 저장할 수 없다. 
		 * - 추출시 형변환 없이 바로 사용 형식] 
		 * 컬렉션<저장할타입> 변수 = new 컬렉션<저장할타입>();
		 */
		ArrayList<String> list2 = new ArrayList<>();
		list2.add("hello");
//		list2.add(1); 에러남 --> String만 저장되므로 error
		ArrayList<Employee> list3 = new ArrayList<>();
		list3.add(new Employee("1","1",100000000));
		System.out.println(list3.indexOf(list3)); //못찾으면 -1
